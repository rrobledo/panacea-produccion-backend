...
| `STOCK_FALTANTES_BLOQUEA_INICIO` | Whether starting an orden de producción requires enough physical stock. **Defaults to `false`** (check currently disabled on purpose): the orden starts anyway and the insumo can go negative. RESERVA on generation and CONSUMO on start are recorded either way. Set to `true` to require it again. |
